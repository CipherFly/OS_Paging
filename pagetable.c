...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		//pointer to pte that will be evicted from memory
		pgtbl_entry_t *temp = coremap[frame].pte;	

		//if evicted pte is not written on swap file
		if (temp->frame & PG_DIRTY){
			evict_dirty_count++;
			int offset = swap_pageout(frame, temp->swap_off); //write to swap
			temp->swap_off = offset;
		
		} else {
			evict_clean_count++;
		}
		
		//temp frame is invalid, clean, unreferenced, and on swap
		temp->frame &= (~PG_VALID);
		temp->frame &= (~PG_DIRTY);
		temp->frame &= (~PG_REF);
		temp->frame |= PG_ONSWAP;
...
char *find_physpage(addr_t vaddr, char type) {
	...
	// check if last bit is valid, init_second_level if not
	...
	// this makes p point to the pagetable entry
	pgtbl_entry_t *lookup = (pgtbl_entry_t *) (pgdir[idx].pde & PAGE_MASK);
	unsigned index = PGTBL_INDEX(vaddr);
	p = &lookup[index];
	...
	if (!(p->frame & PG_VALID)){ // if it is not valid 
		...
		if (!(p->frame & PG_ONSWAP)){ // not on swap
			...
			//put pte in memory
			int mem_frame = allocate_frame(p);
			init_frame(mem_frame, vaddr);
			p->frame = (mem_frame << PAGE_SHIFT);
			//set dirty bit so it is written to swap when evicted
			p->frame |= PG_DIRTY;
			...
			//dirty bit is not set unless modified
			p->frame &= (~PG_DIRTY);
			p->frame &= (~PG_ONSWAP);
		}
	} else {
		hit_count++; // increment hit count when valid (in memory)
		ref_count++; 
	}
