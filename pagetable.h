...
#define BIG_TRACE_MASK  (~(0xff000000000)) //37th bit onwards is set to 0

#define PGTBL_MASK        (PTRS_PER_PGTBL-1)
#define PGDIR_INDEX(x)   ((x & BIG_TRACE_MASK) >> PGDIR_SHIFT)
#define PGTBL_INDEX(x)   (((x) >> PAGE_SHIFT) & PGTBL_MASK)

#define PAGE_FRAME(x)       (x << (PAGE_SHIFT - 4))
#define DIR_FRAME(x)       (x << (PGDIR_SHIFT - 4))
